...
public class Philosopher extends Thread {
	...
	private void philosopher_walk()
	{
		int walkTime = 1 + new Random().nextInt(8000);
		System.out.println("Philosopher " + id + " has started walking.");
		try {
			sleep(walkTime);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public void wait_otherphilosophs(int i)
	{
		System.out.println("Philosopher " + id + " is waiting for other philosophers.");
		for (int j = 0; j < N; j++)
		{
			// Barrier implementation for all philosophers to wait for each other.
			barriers[i].release();
		}
	...
	{
		int thinking = new Random().nextInt(8000);
	...
	public static void main(String args[]) {
		...
		for(int i = 0; i < N; i++){
			try{
				table.PutPlate_GUI(oldies[i].id);
				bararray[i].acquire();
	...
	public void run() {
			...
			philosopher_walk();
			wait_otherphilosophs(id);
			System.out.println("Philosopher " + id + " has stopped waiting for other philosophers.");

			try {
				sleep(500); // To indicate the initial condition in GUI.

				while (true)
				{
					think(id);
					System.out.println("Philosopher " + id + " is getting hungry.");
					take_forks(id);
